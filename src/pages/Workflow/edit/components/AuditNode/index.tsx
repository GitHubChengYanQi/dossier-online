...
type AuditNodeProps = {
    onUserRender?: (users: RestUserResult[]) => void
}

const AuditNode: React.FC<AuditNodeProps> = ({onUserRender}) => {

    const {auditExtend} = useContext(WFC);
    ...
    return (
        <ProFormDependency name={['type']}>
            {({type}) => {
                ...
                if (findEle(type, 'ASSIGNER')) {
                    items.push(
                        <Form.Item
                            ...
                            help={'选定的人员可以进行操作'}
                        >
                            <SelectUser onRender={(users: RestUserResult[]) => {
                                onUserRender?.(users);
                            }}/>
                        </Form.Item>,
                    );
                ...
                if (findEle(type, 'EXTEND')) {
                    items.push(<RenderField
                        config={{...auditExtend, dataIndex: "extend"}}/>)
                }
                return items;
            }}
        </ProFormDependency>

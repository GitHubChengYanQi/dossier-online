            ...
            <DrawerForm<AuditNodeType>
                ...
                onFinish={async (values) => {
                    ...
                    props.objRef.remark = (values.conditions || []).map((item: conditionType) => {
                        const render: any = renderList.find((listItem: any) => listItem.key === item.value) || {};
                        return ((condition || []).find((conditionItem: conditionType) => conditionItem.fieldName === item.fieldName)?.fieldTitle || '') + '  ' + (conditionEnum[item.condition]?.text || '') + '  ' + (render.title || '');
                    });
                    updateNode?.();
                    setOpen(false);
                }}
